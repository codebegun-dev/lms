...
//   // ✅ FIXED: Include all required fields that backend expects
//   const handleSave = async () => {
//     const newErrors = {};
//     if (!formData.parentMobileNumber) newErrors.parentMobileNumber = "Parent mobile is required";
//     if (!formData.dateOfBirth) newErrors.dateOfBirth = "DOB is required";
...
//       // ✅ FIX: Include ALL required fields that backend expects
//       const payload = {
//         userId: userId,
//         firstName: formData.firstName, // Required by backend
//         lastName: formData.lastName,   // Required by backend  
//         email: formData.email,         // Required by backend
//         phone: formData.mobileNumber,  // Required by backend (note: phone not mobileNumber)
//         gender: formData.gender,
//         dateOfBirth: formData.dateOfBirth,
//         parentMobileNumber: formData.parentMobileNumber,
//         bloodGroup: formData.bloodGroup,
...
const PersonalInformation = () => {
  ...
  // ✅ Function to update localStorage user data
  const updateLocalStorageUser = (newProfileData) => {
    ...
    const updatedUser = {
      ...
      phone: newProfileData.mobileNumber,
      profilePicturePath: newProfileData.profilePicturePath
    };
    localStorage.setItem("user", JSON.stringify(updatedUser));
    
    // ✅ Trigger navbar update
    window.dispatchEvent(new CustomEvent("profile-updated"));
    console.log("LocalStorage user updated with profile picture:", newProfileData.profilePicturePath);
  };
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        ...
        // ✅ Update localStorage when loading data
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ✅ CRITICAL: Update localStorage after successful save
      updateLocalStorageUser(res.data);
  ...
  const removeProfilePicture = async () => {
    ...
    try {
      ...
      // ✅ CRITICAL: Update localStorage after removing profile picture
      updateLocalStorageUser(res.data);
