...
//   const [question, setQuestion] = useState("Click Start to load the first question.");
//   const [transcript, setTranscript] = useState("");
//   const [showNextButton, setShowNextButton] = useState(false); // 👈 added
...
//     await startRecording();
//     setShowNextButton(true); // 👈 show Next button after starting
//   };
...
export default function StartInterview() {
  ...
  // ✅ FIXED handleStart — shows question immediately
  // const handleStart = async () => {
  //   try {
  ...
  //     // ✅ Then start recording and show Next button
  //     await startRecording();
  ...
  const handleStart = async () => {
  try {
    setQuestion("Loading your first question..."); // show placeholder instantly
    ...
    if (!activeInterviewId) {
      ...
      // fetch interview start + question
      const res = await axios.post("http://localhost:8080/api/interviews/start", requestDto);
      activeInterviewId = res.data?.interview?.interviewId;
      setInterviewId(activeInterviewId);

      // ✅ Display first question immediately (faster update)
      const firstQ =
        res.data?.firstQuestion?.title ||
        res.data?.firstQuestion ||
        "First question not provided";
      ...
      // ✅ Already have interviewId — fetch first question separately
      const res = await axios.get(
    ...
    // ✅ Start recording and show next button (after question shown)
    await startRecording();
    setShowNextButton(true);
  } catch (err) {
    console.error("start interview api error", err);
