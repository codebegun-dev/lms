...
// import React, { useState, useEffect } from "react";
// import { FaEye, FaEdit, FaTrash, FaBan, FaTimes, FaArrowLeft, FaSave, FaUserPlus, FaShieldAlt, FaKey } from "react-icons/fa";
// import axios from "axios";
...
//     try {
//       // This will get only Admin and Interviewer roles from backend
//       const response = await axios.get("http://localhost:8080/api/roles");
      
//       // Filter out STUDENT role just to be safe (even though backend should handle it)
//       const filteredRoles = response.data.filter(role => 
//         role.name !== "STUDENT" && role.name !== "MASTER_ADMIN"
//       );
...
//     } catch (error) {
//       console.error("Error loading roles:", error);
//       // Fallback to default roles if API fails (only Admin and Interviewer)
//       setAvailableRoles([
//         { name: "ADMIN" },
//         { name: "INTERVIEWER" }
//       ]);
...
//                 Available roles for assignment: Admin and Interviewer only
//               </div>
//             </div>
//           </div>
...
//     try {
//       const response = await axios.get("http://localhost:8080/api/roles");
//       // Filter out STUDENT role for editing as well
//       const filteredRoles = response.data.filter(role => 
//         role.name !== "STUDENT" && role.name !== "MASTER_ADMIN"
//       );
//       setAvailableRoles(filteredRoles);
...
//       await axios.put(`http://localhost:8080/api/user/${user.id}`, payload);
//       alert("User updated successfully!");
//       onUpdate({ ...user, ...formData });
//       setIsEditing(false);
//     } catch (error) {
...
//                 <div>
//                   <span className={`badge ${
//                     formData.role === "ADMIN" ? "bg-danger" : 
//                     formData.role === "INTERVIEWER" ? "bg-success" : "bg-info"
//                   } fs-6`}>
//                     {formData.role}
//                   </span>
//                   <span className={`badge ${
...
//   // Hardcoded filter options - shows ALL roles including Student
//   const filterOptions = ["All", "ADMIN", "STUDENT", "INTERVIEWER"];
