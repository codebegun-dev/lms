...
const ContentTypeModal = ({ show, onClose, onSelectType }) => {
                ...
                {contentTypes.map((item, index) => (
                  <div key={item.type} className="col-6">
                    {item.type === "Coding lab" && (
                      <div className="border-0  pt-3 mb-2"></div>
                    )}
                    
                    <div
                      className={`card border-0 cursor-pointer ${
                        item.featured ? "bg-light" : "bg-white"
                      }`}
                      style={{ 
                        cursor: 'pointer',
                      ...
                      onMouseLeave={(e) => {
                        e.currentTarget.style.backgroundColor = item.featured ? '#f8f9fa' : '#fff';
  ...
  {contentTypes.map((item, index) => (
    <div key={item.type} className="col-6">
      ...
      <div
        className="card border-0 cursor-pointer bg-light"
        style={{ 
          cursor: 'pointer',
          transition: 'all 0.2s ease'
        }}
...
<div className="row g-3">
  ...
  {/* Add a sub-section - Full width */}
  <div className="col-12">
    <div className="border-top pt-3 mt-2"></div>
    <div
      className="card border cursor-pointer bg-white"
      style={{ 
        cursor: 'pointer',
      ...
      onClick={() => handleTypeSelect("Sub-section")}
      onMouseEnter={(e) => {
        e.currentTarget.style.backgroundColor = '#f8f9fa';
        e.currentTarget.style.transform = 'translateY(-2px)';
        e.currentTarget.style.boxShadow = '0 4px 8px rgba(0,0,0,0.1)';
    ...
    >
      <div className="card-body p-3">
        <div className="d-flex align-items-center">
          ...
          <div className="flex-grow-1">
            <h6 className="mb-1 fw-bold">Add a sub-section</h6>
            <p className="text-muted small mb-0">
