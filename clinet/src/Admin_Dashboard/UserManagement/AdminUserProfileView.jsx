...
const AdminUserProfileView = ({ user, onClose }) => {
  ...
  const fetchCoursesAndBatches = async () => {
    try {
      ...
      console.error("Error fetching courses/batches:", err);
      showToast("Failed to load courses and batches", "danger");
    }
  };
  ...
  const loadAllSections = async () => {
    try {
      ...
      // Always load basic info - handle different user structures
      loaded.basic = {
        firstName: user.firstName || "",
        lastName: user.lastName || "",
        email: user.email || "",
        ...
        role: user.role?.name || user.role || "",
        status: user.status || "active",
        // Add userId to basic info for reference
  ...
  const loadStudentSections = async (loaded) => {
    try {
      // Get userId from user object - handle different structures
      const userId = user.userId || user.id;
      
      if (!userId) {
        console.error("No user ID found");
        showToast("User ID not found", "danger");
        return;
      }

      console.log("Loading student sections for userId:", userId); // Debug log
      ...
      try {
        ...
        console.log("No personal info found:", err.message);
        loaded.personal = {};
      }
      ...
      try {
        ...
        console.log("No generic details found:", err.message);
        loaded.generic = {};
      }
      ...
      try {
        ...
        console.log("No 10th grade details found:", err.message);
        loaded.tenth = {};
      }
      ...
      try {
        ...
        console.log("No 12th grade details found:", err.message);
        loaded.twelfth = {};
      }
      ...
      try {
        ...
        console.log("No UG details found:", err.message);
        loaded.ug = {};
      }
      ...
      try {
        ...
        console.log("No PG details found:", err.message);
        loaded.pg = {};
      }
      ...
      try {
        ...
        console.log("No course details found:", err.message);
      }
      ...
      try {
        ...
        console.log("No fee details found:", err.message);
      }

      loaded.courseFee = {
        courseId: courseData.courseId || "",
        courseName: courseData.courseName || "Not assigned",
        batchId: courseData.batchId || "",
        batchName: courseData.batchName || "Not assigned",
        startDate: courseData.startDate || "",
        totalFee: feeData.totalFee || 0,
        paidFee: feeData.paidFee || 0,
        balanceFee: feeData.balanceFee || 0
      };
      
    } catch (err) {
      console.error("Error loading student sections:", err);
      showToast("Failed to load student sections", "danger");
    }
  };
  ...
  const loadAdminSections = async (loaded) => {
    try {
      ...
      try {
        ...
        console.log("No admin personal info found:", err.message);
        loaded.adminPersonal = {};
      }
    } catch (err) {
      console.error("Error loading admin sections:", err);
  ...
  const loadSalesSections = async (loaded) => {
    try {
      ...
      try {
        ...
        console.log("No sales personal info found:", err.message);
        loaded.salesPersonal = {};
      }
    } catch (err) {
      console.error("Error loading sales sections:", err);
  ...
  const loadInterviewerSections = async (loaded) => {
    try {
      ...
      try {
        ...
        console.log("No interviewer personal info found:", err.message);
        loaded.interviewerPersonal = {};
      }
    } catch (err) {
      console.error("Error loading interviewer sections:", err);
  ...
  const toggleEdit = (key) => {
    ...
    if (key === "courseFee" && !["ADMIN", "MASTER_ADMIN"].includes(currentAdminRole.toUpperCase())) {
      showToast("Only ADMIN/MASTER_ADMIN can edit Course & Fee details.", "warning");
      return;
    }
  ...
  const handleSave = async (key, data) => {
    try {
      ...
      // Remove userId from payload if it's already in data (to avoid duplication)
      ...
      switch (key) {
        ...
        case "courseFee":
          ...
          setSectionData((p) => ({ ...p, [key]: data }));
          setEditMode((p) => ({ ...p, [key]: false }));
          showToast("Course & Fee details updated successfully!", "success");
          return;
        case "adminPersonal":
          endpoint = `${API_BASE}/api/student/personal-info/update`;
          break;
        ...
        default:
          console.warn(`No endpoint defined for section: ${key}`);
          showToast(`No update endpoint for ${key}`, "warning");
          return;
      }
      ...
      if (keyName === "projects" && Array.isArray(formState)) {
        if (formState.length === 0) {
          return (
            <div className="text-center py-5">
              <div style={{ fontSize: "4rem", opacity: 0.3 }}>📁</div>
              <p className="text-muted mb-0 mt-2">No projects found</p>
            </div>
          );
        }
          ...
          <div className="bg-white rounded-4 shadow-lg" style={{ maxWidth: "1200px", margin: "0 auto" }}>
            <div className="p-4 border-bottom d-flex justify-content-between align-items-center" style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" }}>
              <div className="text-white">
                ...
                <div className="d-flex gap-2 flex-wrap">
                  <span className="badge bg-white text-dark px-3 py-2">{userRole}</span>
                  <span className="badge bg-white text-dark px-3 py-2">{user.email}</span>
                  <span className="badge bg-white text-dark px-3 py-2">ID: {user.userId || user.id}</span>
                </div>
              </div>
            ...
            <div className="p-4" style={{ maxHeight: "60vh", overflowY: "auto" }}>
              {loading ? (
                ...
                <div className="text-center py-5">
                  <div style={{ fontSize: "4rem", opacity: 0.3 }}>👤</div>
                  <p className="text-muted mb-0 mt-2">No sections available for this user role</p>
                </div>
              )}
            </div>
