...
// QuestionCard.jsx (simplified)
import React from "react";
import { Card, Badge } from "react-bootstrap";

export default function QuestionCard({ data, index, onEdit, onDelete }) {
  return (
    <Card className="mb-3 shadow-sm">
      <Card.Body>
        <div className="d-flex justify-content-between align-items-start">
          <div className="flex-grow-1">
            <div className="d-flex align-items-center mb-2">
              ...
              <Badge bg={
                data.type === 'single' ? 'primary' :
                data.type === 'multiple' ? 'success' :
                data.type === 'manual' ? 'warning' : 'info'
              } className="me-2">
                {data.type}
              </Badge>
              <Badge bg="success" className="me-2">+{data.positiveMarks || 0}</Badge>
              {data.negativeMarks > 0 && (
                <Badge bg="danger">-{data.negativeMarks}</Badge>
              )}
              {data.mandatory && (
                <Badge bg="warning" text="dark">Mandatory</Badge>
              )}
            </div>
            <h6 className="mb-2">{data.question}</h6>
          </div>
