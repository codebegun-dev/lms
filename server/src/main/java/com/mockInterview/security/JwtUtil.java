...
public class JwtUtil {
    ...
    private final Key key = Keys.secretKeyFor(SignatureAlgorithm.HS512);
    private final long jwtExpirationMs = 3600000L; // 1 hour in milliseconds
    ...
    public String generateToken(Long userId, String email, List<String> roles, List<String> permissions) {
        return Jwts.builder()
                .setSubject(email)
                .claim("userId", userId)
                .claim("roles", roles == null ? new ArrayList<>() : roles)
                .claim("permissions", permissions == null ? new ArrayList<>() : permissions)
                .setIssuedAt(new Date())
                .setExpiration(new Date(System.currentTimeMillis() + jwtExpirationMs))
                .signWith(key)
                .compact();
    ...
    // return list of permissions claim (may be empty)
    public List<String> getPermissionsFromToken(String token) {
        return getListClaim(token, "permissions");
    }
